...
public class AssestController {
    ...
    @PostMapping("/add/{id}")
    public ResponseEntity addAssest(@PathVariable int id,@Valid @RequestBody Assest assest, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateAssest(@Valid @RequestBody Assest assest, @PathVariable int id,Errors errors) {
        if (errors.hasErrors()) {
            String errorMessage = errors.getAllErrors().get(0).getDefaultMessage();
            return ResponseEntity.status(400).body(errorMessage);
        }
