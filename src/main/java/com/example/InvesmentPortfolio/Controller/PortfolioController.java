...
public class PortfolioController {
    ...
    @PostMapping("/add")
    public ResponseEntity addPortfolio(@Valid @RequestBody Portfolio portfolio,Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updatePortfolio(@PathVariable Integer id,@Valid @RequestBody Portfolio portfolio,Errors errors) {
        if (errors.hasErrors()) {
            String errorMessage = errors.getAllErrors().get(0).getDefaultMessage();
            return ResponseEntity.status(400).body(errorMessage);
        }
